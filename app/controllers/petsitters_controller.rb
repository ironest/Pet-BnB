class PetsittersController < ApplicationController
    ...
    def index
        if current_user
            # If a user is logged in, make sure to exclude himself from the potential pet-sitters
            @petsitters = Petsitter.where.not(user_id: current_user.id)
        else
            # If a user is not logged in, we cannot identify whether or not this user is an pet-sitter
    ...
    def create
        whitelisted_params = petsitter_params
        # Create a new petsitter record (using information and FK generated by current_user)
